...
router.get('/', async (req, res) => {
  try {
    ...
    //res.render('dasboard/dashboard', { tasks: allTasks });
    res.json(allTasks);
  } catch (e) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.status(404).json({ error: `${e}: Post not found` });
  }
});
...
router.patch('/:id', async (req, res) => {
  ...
  try {
    ...
    if (updatedTaskData.dueDate && updatedTaskData.dueDate !== oldTask.dueDate)
    ...
    //Adding subtask
    if (updatedTaskData.subTask && updatedTaskData.subTask !== oldTask.subTask)
      updatedTaskObject.subTask = updatedTaskData.subTask;
    //Adding comment
    if (updatedTaskData.comment && updatedTaskData.comment !== oldTask.comment)
...
router.delete('/:id', async (req, res) => {
  if (!req.params.id) {
    res.status(400).json({ error: 'You must supply and ID to delete a task' });
    return;
  }
  try {
    await tasksData.getTaskById(req.params.id);
