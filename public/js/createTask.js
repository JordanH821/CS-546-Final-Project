...
function addSubtasksToForm() {
  ...
  if (length === 0) {
    ...
    $('#subtaskList li').each((index, subtask) => {
      form.append(
        `<input type="text" name="subtasks[0]" value="${$(subtask).text()}">`
      );
    });
  } else {
    $('#subtaskList li').each((index, subtask) => {
...
function addDependenciesToForm() {
  ...
  if (length === 0) {
    ...
    $(dependencies).each((index, dependency) => {
      form.append(
        `<input type="text" name="dependencies[0]" value="${$(dependency).data(
          'id'
        )}">`
      );
    });
    ...
    $(dependencies).each((index, dependency) => {
      form.append(
        `<input type="text" name="dependencies" value="${$(dependency).data(
          'id'
        )}">`
      );
    });
...
$('#taskForm').on('submit', event => {
  ...
  try {
    ...
    valid = false;
    displayError('Title cannot be empty. Please type in a title for this task');
  }
  try {
    validateStringInput(
      $('#description')
